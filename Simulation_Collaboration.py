...
class simulateOnlineData(object):
	...
	def runAlgorithms(self, algorithms):
		...
		# Training upper left and lower left
		...
		partition = int(len(self.users)*0.5)
		# Training lower right
		for iter_ in range(len(self.articles) - self.training_iterations):
			article = self.articles[iter_+self.training_iterations]										
			for u in self.users[partition:]:
				noise = self.noise()	
				reward = self.getReward(u, article)
				reward += noise										
				for alg_name, alg in algorithms.items():
...
if __name__ == '__main__':
	...
	# algorithms['PTS'] = PTSAlgorithm(particle_num = 30, dimension = 10, n = n_users, itemNum=n_articles, sigma = np.sqrt(.5), sigmaU = 1, sigmaV = 1)
